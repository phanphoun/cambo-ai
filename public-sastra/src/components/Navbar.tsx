  ...
  return (
    <nav
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 select-none ${
        scrolled
          ? "bg-[#0A0805]/95 border-b border-[#3C2D18] backdrop-blur-xl shadow-2xl shadow-black/80 py-3"
          : "bg-transparent py-5"
      }`}
    >
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex items-center justify-between">
        ...
        <a href="#" className="flex items-center gap-3 group">
          <div className="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-[#2D2111] via-[#1A140C] to-[#100C07] border border-gold/60 text-gold shadow-lg shadow-black/80 p-1 group-hover:scale-105 transition-all">
            <KhmerLotusMedallion className="h-full w-full" glow />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <span className="font-heading font-bold text-lg sm:text-xl text-gold-gradient tracking-tight">
                SASTRA AI
              </span>
        ...
        {/* Desktop Nav Links */}
        <div className="hidden lg:flex items-center gap-7 text-xs font-semibold text-stone-300 font-khmer">
          ...
          <a href="#use-cases" className="hover:text-gold transition-colors">
            {t.nav.useCases}
          </a>
        ...
        <div className="hidden sm:flex items-center gap-3">
          ...
          <div className="relative">
            ...
            {langMenuOpen && (
              <div className="absolute right-0 mt-2 w-38 rounded-2xl border border-[#3C301D] bg-[#14100C] p-1.5 shadow-2xl shadow-black z-30 animate-fade-in font-sans">
                <button
                  type="button"
                  onClick={() => {
                    setLanguage("km");
          ...
          <a
            ...
            className="flex items-center gap-2 rounded-2xl border border-gold/60 bg-gradient-to-r from-gold via-amber-500 to-amber-700 px-5 py-2 text-xs font-bold text-black shadow-lg shadow-gold/20 hover:brightness-110 hover:scale-[1.02] active:scale-[0.98] transition-all cursor-pointer glow-gold"
          >
      ...
      {/* Mobile Dropdown */}
      {mobileOpen && (
        <div className="lg:hidden border-b border-[#3C2D18] bg-[#0E0C09] px-4 py-5 space-y-4 animate-fade-in shadow-2xl">
